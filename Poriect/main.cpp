...
struct Triangles{int color;int piecesNumber;int quarter;}triangle[25];
int board[7][14];
...
void setup()
{
    ...
    for(i=1;i<=24;i++)
        if(i<=6)
        triangle[i].quarter=3;
        if(i=6&&i<=12)
        triangle[i].quarter=4;
        if(i>12&&i<=18)
        triangle[i].quarter=2;
        if(i>18&&i<=24)
        triangle[i].quarter=1;
            ...
            if(board[i][j]==54)
            ...
            else if(board[i][j]==41)
                {
                    ...
                    cout<<" "<<triangle[board[1][j]].piecesNumber<<"W";
                            ...
                            if(j==0)
                    cout<<" "<<triangle[board[5][j]].piecesNumber<<"W";
                    else cout<<triangle[board[5][j]].piecesNumber<<"W";
                    }
                }
...
int main ()
{
    int i,j,dice1,dice2,nr1,nr2,player=1;
    ...
    while (1)
    {
        ...
        getline (cin,passd);diceRoll(dice1,dice2);
        ...
        if(((passd.substr(0,1)>="1"&&passd.substr(0,1)<="9")||(passd.substr(0,2)>="10"&&passd.substr(0,2)<="24"))&&((passd.substr(2,1)>="1"&&passd.substr(2,1)<="9")||(passd.substr(2,2)>="10"&&passd.substr(2,2)<="24")))
        {
            if(passd.length()==4)
            {
                ...
                nr1=stoi(half1);nr2=stoi(half2);
            }
            if(passd.length()==3)
            {
                if(passd.substr(0,2)<="24"&&passd.substr(0,2)>="10")
            ...
            if(checkMoveLegal(dice1,dice2,nr2-nr1)==true)
                if(triangle[nr1].color==player)
                    if(triangle[nr2].piecesNumber==0||triangle[nr2].color==player)
                        {
                            if(player==1)
                            {
                                if(triangle[nr1].quarter>=triangle[nr2].quarter)
                                    triangle[nr2].piecesNumber++;
                                    if(triangle[nr2].piecesNumber==1)
                                    {
                                        triangle[nr2].color=player;
                                        if(nr2>12)
                                            board[4][nr2-1]=41;
                                        else board[2][nr2-1]=41;
                                    }
                                    triangle[nr1].piecesNumber--;
                            }
                            else
                            {
                                if(triangle[nr1].quarter<=triangle[nr2].quarter)
            ...
            printBoard();diceRoll(dice1,dice2);
            cout<<"Dice rolls: "<<dice1<<"  "<<dice2;cout<<endl;
        }
